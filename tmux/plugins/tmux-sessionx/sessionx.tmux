...
handle_args() {
	...
	CONFIGURATION_MODE="$bind_configuration_mode:reload(find -L $CONFIGURATION_PATH -path $CONFIGURATION_PATH -prune -o \( -type d -o -type l \) -print)+change-preview($LS_COMMAND {})"
	...
	NEW_WINDOW="$bind_new_window:reload(find -L $PWD -path $PWD -prune -o \( -type d -o -type l \) -print)+change-preview($LS_COMMAND {})"
